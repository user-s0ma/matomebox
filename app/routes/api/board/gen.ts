...
export async function action({ request }: Route.ActionArgs) {
  if (request.method === "POST") {
    ...
    ユーザーからの指示と、提供された既存のボードアイテムの情報を分析し、情報を豪華かつ視覚的に分かりやすく再構成・追記してください。
    カラフルでポップな見た目にしてください。
    ...
    以下の指示に従って、新しいボードアイテムを生成してください:
    1.  **情報の整理と構造化:** ユーザーの指示と既存のコンテキストから主要なテーマ、アイデア、タスク、関連情報などを抽出し、それらを論理的にグループ化・階層化してください。
    2.  **視覚的な魅力:**
        *   **付箋 (note):** 重要なポイントやアイデアを付箋で表現します。関連性の高い情報は同じような色の付箋にするか、テーマごとに色分けするなど、色彩を効果的に使用してください。内容に合わせて付箋のサイズを調整しても構いません。
    ...
    3.  **配置の工夫:** 新しく生成するアイテムは、既存のアイテムや他の生成アイテムと重ならないように、かつ関連性が視覚的に伝わるように配置してください。ボード全体を効果的に使い、見やすいレイアウトを心がけてください。座標 (x, y) は、一般的なホワイトボードの感覚で、0から数千の範囲で適切に設定してください。
